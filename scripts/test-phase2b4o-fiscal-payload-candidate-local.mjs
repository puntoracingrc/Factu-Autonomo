...
function main() {
  ...
  const testFile = path.join(
    "scripts",
    "phase2b4o-fiscal-payload-candidate-local.test.ts",
  );

  run(vitestBin, ["run", testFile], {
    env: {
      ...process.env,
      PHASE2B4O_LOCAL_ACCEPTANCE: "1",
      PHASE2B4O_SUPABASE_URL: status.API_URL,
      PHASE2B4O_SUPABASE_DB_URL: status.DB_URL,
      PHASE2B4O_SUPABASE_ANON_KEY: status.ANON_KEY,
      PHASE2B4O_SUPABASE_ADMIN_KEY: status.SERVICE_ROLE_KEY,
    },
  });

  console.log("Phase 2B.4O fiscal payload candidate local acceptance passed.");
  console.log("Cases: alta payload, chained payload, anulacion payload, chain mismatch, missing recordHash, no transport, no final AEAT XML.");
