...
function main() {
  ...
  const testFile = path.join(
    "scripts",
    "phase2b4q-fiscal-evidence-packet-local.test.ts",
  );

  run(vitestBin, ["run", testFile], {
    env: {
      ...process.env,
      PHASE2B4Q_LOCAL_ACCEPTANCE: "1",
      PHASE2B4Q_SUPABASE_URL: status.API_URL,
      PHASE2B4Q_SUPABASE_DB_URL: status.DB_URL,
      PHASE2B4Q_SUPABASE_ANON_KEY: status.ANON_KEY,
      PHASE2B4Q_SUPABASE_ADMIN_KEY: status.SERVICE_ROLE_KEY,
    },
  });

  console.log("Phase 2B.4Q fiscal evidence packet local acceptance passed.");
  console.log("Cases: alta evidence, chained evidence, anulacion evidence, invalid payload rejection, no transport, no final AEAT XML.");
