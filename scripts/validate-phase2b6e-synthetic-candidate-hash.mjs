...
if (
  packageJson.scripts?.["validate:phase2b6e-synthetic-candidate-hash"] !==
  "node scripts/validate-phase2b6e-synthetic-candidate-hash.mjs"
) {
  fail("Missing npm script validate:phase2b6e-synthetic-candidate-hash.");
}

const hashSource = [
  read("src/lib/verifactu-candidate-pipeline/types.ts"),
  read("src/lib/verifactu-candidate-pipeline/candidate-hash.ts"),
].join("\n");
for (const marker of [
  "node:crypto",
  "sha256-candidate-v1",
  "officialFingerprint: false",
  "buildSyntheticCandidateHash",
]) {
  if (!hashSource.includes(marker)) fail(`Missing hash marker ${marker}.`);
}
if (/officialFingerprint\s*:\s*true/i.test(hashSource)) {
  fail("Candidate hash must not be marked as an official fingerprint.");
}
...
try {
  execFileSync("npx", ["vitest", "run", "src/lib/verifactu-candidate-pipeline/candidate-hash.test.ts"], {
    cwd: root,
    encoding: "utf8",
    stdio: "pipe",
  });
} catch (error) {
  fail(`Candidate hash tests failed: ${error.message}`);
}

if (errors.length > 0) {
  console.error("Phase 2B.6E synthetic candidate hash validation failed:");
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}

console.log("Phase 2B.6E synthetic candidate hash validation passed.");
