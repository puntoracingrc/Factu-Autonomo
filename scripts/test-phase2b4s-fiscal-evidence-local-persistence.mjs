...
function main() {
  ...
  if (!functionExists(status.DB_URL, "create_fiscal_record_with_chain_local_staging")) {
    applySql(status.DB_URL, chainMigrationFile);
  }
  applySql(status.DB_URL, evidenceMigrationFile);
  if (!functionExists(status.DB_URL, "create_fiscal_evidence_packet_local_staging")) {
    fail("create_fiscal_evidence_packet_local_staging does not exist locally.");
  }

  const vitestBin = path.join(root, "node_modules", ".bin", "vitest");
  const testFile = path.join(
    "scripts",
    "phase2b4s-fiscal-evidence-local-persistence.test.ts",
  );

  run(vitestBin, ["run", testFile], {
    env: {
      ...process.env,
      PHASE2B4S_LOCAL_ACCEPTANCE: "1",
      PHASE2B4S_SUPABASE_URL: status.API_URL,
      PHASE2B4S_SUPABASE_DB_URL: status.DB_URL,
      PHASE2B4S_SUPABASE_ANON_KEY: status.ANON_KEY,
      PHASE2B4S_SUPABASE_ADMIN_KEY: status.SERVICE_ROLE_KEY,
    },
  });

  console.log("Phase 2B.4S fiscal evidence local persistence acceptance passed.");
  console.log("Cases: alta, chained alta, anulacion, idempotent persistence, invalid payload rejection, no transport attempts.");
  console.log("Environment: local Supabase only.");
  console.log("Note: immutable local fiscal records and evidence rows remain for this test prefix; reset local Supabase to clean them.");
}
