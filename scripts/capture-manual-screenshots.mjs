...
async function shot(page, name, options = {}) {
  ...
  try {
    if (options.locator) {
      const el = page.locator(options.locator).first();
...
async function main() {
  ...
  await waitForApp(page);
  await shot(page, "inicio-trimestre", {
    locator: "section[aria-labelledby='quarter-summary-heading']",
  });
  await shot(page, "inicio-acumulado", {
    locator: "section[aria-labelledby='global-summary-heading']",
  });
  await shot(page, "inicio-accesos-rapidos", {
    locator: "text=¿Qué quieres hacer?",
  });
  ...
  await waitForApp(page);
  await shot(page, "facturas-pdf", { locator: "text=F-2026-0002" });
  await shot(page, "facturas-enviar", { locator: "text=F-2026-0002" });
  await shot(page, "facturas-cobrar", { locator: "text=F-2026-0002" });
  await shot(page, "facturas-rectificar", { locator: "text=F-2026-0003" });
  ...
  await waitForApp(page);
  await shot(page, "presupuestos-aceptado", { locator: "text=P-2026-0001" });

  await page.goto(`${BASE_URL}/facturas`, { waitUntil: "networkidle" });
  await waitForApp(page);
  await shot(page, "recibos-automatico", { locator: "text=Recibo creado" });
