...
if (
  packageJson.scripts?.["validate:phase2b6d-synthetic-canonicalization"] !==
  "node scripts/validate-phase2b6d-synthetic-canonicalization.mjs"
) {
  fail("Missing npm script validate:phase2b6d-synthetic-canonicalization.");
}

const canonicalSource = [
  read("src/lib/verifactu-candidate-pipeline/types.ts"),
  read("src/lib/verifactu-candidate-pipeline/canonicalize.ts"),
].join("\n");
for (const marker of [
  "phase2b6d-candidate-canonical-v1",
  "candidate_not_aeat",
  "SYNTHETIC_ONLY_",
  "canonicalizeSyntheticCandidateInput",
]) {
  if (!canonicalSource.includes(marker)) fail(`Missing canonical marker ${marker}.`);
}
if (canonicalSource.includes("FactuAutonomoSyntheticFiscalCandidate")) {
  fail("Canonicalization must not build XML.");
}
...
for (const relativePath of listModuleFiles()) {
  ...
  for (const [label, term] of [
    ...
    ["service role literal", joinTerms("service", "_role")],
  ...
  if (/transportable\s*:\s*true/i.test(source)) {
...
try {
  execFileSync("npx", ["vitest", "run", "src/lib/verifactu-candidate-pipeline/canonicalize.test.ts"], {
    cwd: root,
    encoding: "utf8",
    stdio: "pipe",
  });
} catch (error) {
  fail(`Canonicalization tests failed: ${error.message}`);
}

if (errors.length > 0) {
  console.error("Phase 2B.6D synthetic canonicalization validation failed:");
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}

console.log("Phase 2B.6D synthetic canonicalization validation passed.");
