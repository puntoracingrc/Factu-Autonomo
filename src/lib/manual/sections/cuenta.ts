  ...
  steps: [
    ...
    },
    {
      ...
      paragraphs: [
        ...
        "Para restaurar basta una única confirmación explícita. Al pulsar **Restaurar con copia automática**, Factu solicita primero la descarga de un JSON del estado exacto que va a reemplazar y, sin esperar a otro paso manual, guarda la copia importada. Solo se bloquea si no puede preparar esa copia de seguridad, si detecta un cambio real durante la operación o si el navegador no puede confirmar la escritura durable; una actualización periódica de metadatos de sincronización no debe provocar un bucle de aviso.",
        ...
        "En **Cuenta → Copias → Mantenimiento · archivar documentos descartados** puedes retirar de las listas activas facturas o recibos que tú identificas expresamente como descartados. Factu no los clasifica ni los selecciona automáticamente. La tarjeta solo se desbloquea para la cuenta owner con email confirmado, fuera de la demo, con la nube en **Sincronizado** y sin cambios pendientes, y con el nivel MFA de la sesión resuelto. Escribe cada número completo y exacto, revisa la vista previa sellada y copia literalmente la frase de confirmación. Inmediatamente antes del único commit durable se generan los bytes de una **copia JSON previa** y se solicita su descarga; el historial guarda hash y tamaño, sin afirmar que el sistema operativo haya terminado de escribir el archivo. Si no puede prepararse o cambian los datos, no se archiva nada. Relaciones ambiguas, referencias externas no compatibles y cualquier evidencia Veri*Factu protegida quedan bloqueadas. Una factura original ya anulada mediante rectificativa puede retirarse de forma explícita sin borrar ni modificar la rectificativa superviviente. La nube conserva intactos los documentos y sincroniza un overlay reversible: los clientes actuales los ocultan de las listas sin enviar borrados físicos ni reescribir sus snapshots/PDF. El historial conserva documentos, eventos y copias para un **rollback** explícito, que exige otra copia previa y su propia frase; restaurar no libera la identidad ni permite reutilizar la numeración. Tras archivar o restaurar, Factu solicita una sincronización inmediata y el resultado queda visible en la tarjeta.",
      ],
    },
    {
