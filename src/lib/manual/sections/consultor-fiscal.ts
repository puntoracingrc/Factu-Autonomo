...
export const consultorFiscalSection: ManualSection = {
  ...
  steps: [
    ...
    },
    {
      ...
      paragraphs: [
        "Abre **Notificaciones y expedientes** dentro de Asesoría fiscal con una cuenta confirmada. Puedes arrastrar o seleccionar hasta **10 PDF**. Primero aparecen en una cola con nombre, tamaño y estado; nada se analiza automáticamente. Quita los que no correspondan y pulsa una sola vez **Analizar** para procesar el lote de forma local.",
        "Antes de admitir cada PDF, el navegador calcula una huella SHA-256 del contenido y la compara con el lote y con las fichas ya guardadas en esa cuenta. El mismo contenido se rechaza aunque cambie el nombre del archivo. El nombre solo existe mientras ves la cola: no se conserva ni el PDF, ni su nombre, ni su texto completo.",
        ...
        "Después de guardar la ficha puedes elegir **Archivar original en Drive**. La subida es siempre voluntaria y separada del análisis: Factu crea **Factu - documentos oficiales/AAAA/MM** usando exclusivamente la fecha exacta del documento. Si falta, utiliza **Fecha pendiente**; nunca ordena el original por la fecha de escaneo. El PDF permanece en tu Google Drive y Factu solo conserva identificadores opacos, huella, revisión y estado tras releerlo y comprobar que su SHA-256 coincide.",
        "Si vuelves a seleccionar un PDF que ya tiene ficha pero todavía no tiene original archivado, el escáner ofrece **Archivar original en Drive** en lugar de analizarlo otra vez. Si ya está archivado, lo rechaza como duplicado. Para fichas antiguas debes volver a seleccionar el PDF porque Factu no custodia los originales; conectar Drive más tarde no puede recuperarlos automáticamente.",
        ...
        "El reconocimiento actual cubre providencias de apremio, concesiones de aplazamiento o fraccionamiento, diligencias de embargo de bienes inmuebles, requerimientos formales de presentación y acuerdos de alta en el ROI. Si el PDF es una imagen, intenta OCR local en el navegador. Si la lectura no recupera texto suficiente queda como **No reconocido** o **Necesita revisión**; nunca se envía a un proveedor ni a una IA.",
