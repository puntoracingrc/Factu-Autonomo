...
export type RealCorpusRelationTypeV7 =
  ...
  | "ENFORCEMENT_ORDER_PRECEDES_BANK_SEIZURE"
...
function validReference(value: string | null): boolean {
  return (
    ...
    (REFERENCE.test(value) &&
      (/\d/u.test(value) || /^SYN-[A-Z0-9-]+$/u.test(value)) &&
...
function validDocument(value: RealCorpusRelationDocumentV7): boolean {
  ...
  if (
    ...
    (value.opaqueAssetOrdinal !== null &&
      (!Number.isSafeInteger(value.opaqueAssetOrdinal) ||
        value.opaqueAssetOrdinal < 1 ||
        value.opaqueAssetOrdinal > 100))
  )
    return false;
  ...
  for (const installment of value.installments) {
    if (
      ...
      dates.has(installment.dueDate)
    )
      return false;
    dates.add(installment.dueDate);
  }
  if (value.remainingInstallmentBasesCents.some((item) => !validCents(item)))
    return false;
  ...
  for (const row of value.offsetRows) {
    if (
      ...
      !row.debtKey ||
      !validCents(row.beforeCents) ||
      !validCents(row.appliedCents) ||
      !validCents(row.remainingCents) ||
      row.appliedCents > row.beforeCents ||
      row.beforeCents - row.appliedCents !== row.remainingCents ||
      rows.has(row.debtKey)
    )
      return false;
    rows.add(row.debtKey);
  }
  return value.modelsPeriods.every((item) =>
    /^\d{3}:(?:19|20)\d{2}:(?:0A|[1-4]T|0[1-9]|1[0-2])$/u.test(item),
  );
}

function relation(
  status: RealCorpusRelationV7["status"],
  input: Omit<
...
export function decideRealCorpusDuplicateV7(
  source: RealCorpusRelationDocumentV7,
  existing: readonly RealCorpusRelationDocumentV7[],
): RealCorpusDeduplicationDecisionV7 {
  if (
    !validDocument(source) ||
    existing.length > 10_000 ||
    existing.some((item) => !validDocument(item))
  )
    return Object.freeze({
      duplicate: false,
  ...
  const duplicate =
    source.sourceSha256 === null
      ...
      : existing.find(
          (item) =>
            item.ownerScope === source.ownerScope &&
...
export function relateRealCorpusDocumentsV7(
  ...
  const offset =
    source.familyId === "collection.offset_ex_officio" ||
    source.familyId === "collection.offset_requested"
      ? source
      : target.familyId === "collection.offset_ex_officio" ||
          target.familyId === "collection.offset_requested"
        ? target
        : null;
  if (
    offset &&
  ...
  const seizure =
    source.familyId === "seizure.bank_account"
      ? source
      : target.familyId === "seizure.bank_account"
        ? target
        : null;
  if (
    enforcement &&
  ...
  if (
    ...
    source.opaqueAssetOrdinal !== null &&
    target.opaqueAssetOrdinal !== null &&
    source.opaqueAssetOrdinal !== target.opaqueAssetOrdinal
  )
    results.push(
      exact({
  ...
  const original =
    source.familyId === "collection.deferral_grant"
      ...
      : target.familyId === "collection.deferral_grant"
        ...
        : null;
  const modification =
    source.familyId === "collection.deferral_modification"
      ? source
      : target.familyId === "collection.deferral_modification"
        ? target
        : null;
  if (
    original &&
...
export function resolveActivePaymentPlansV7(
  documents: readonly RealCorpusRelationDocumentV7[],
): readonly RealCorpusActivePlanV7[] {
  if (
    documents.length > 1_000 ||
    documents.some((item) => !validDocument(item))
  )
    return Object.freeze([]);
  ...
  for (const document of documents) {
    if (
      (document.familyId === "collection.deferral_grant" ||
        document.familyId === "collection.deferral_modification") &&
      document.agreementId
    ) {
      originals.set(
        `${document.ownerScope}\u0000${document.agreementId}`,
        document,
      );
      result.set(
        document.documentId,
  ...
  for (const modification of documents) {
    if (
      modification.familyId !== "collection.deferral_modification" ||
      !modification.explicitlyModifiesPlan ||
      !modification.replacesAgreementId ||
      !modification.agreementId
    )
      continue;
    const original = originals.get(
      `${modification.ownerScope}\u0000${modification.replacesAgreementId}`,
    );
    if (
      !original ||
      original.debtKey !== modification.debtKey ||
    ...
    result.set(
      ...
      Object.freeze({
        ...
        active: false,
        replacedByDocumentId: modification.documentId,
      }),
    );
  }
