...
function field(input: {
  ...
  return Object.freeze({
    ...
    currency:
      input.amountCents === undefined || input.amountCents === null ? null : "EUR",
...
export function projectRealCorpusReviewV6(
  ...
  if (outcome.status !== "REVIEW_REQUIRED" || !outcome.familyId || !outcome.explanation)
    return emptyReview();
  const rule = resolveFamilyRuleV2(outcome.familyId);
  if (!rule) return emptyReview();
  const paymentPage = outcome.segments.find((segment) => segment.type === "PAYMENT_FORM")?.pageNumbers[0] ?? 1;
  const fields = [
    field({
    ...
    ...(outcome.paymentFormOperationCount > 0
      ? [field({
          ...
          displayValue: "Sirve para pagar; sus copias son una sola operación y no acreditan el pago",
