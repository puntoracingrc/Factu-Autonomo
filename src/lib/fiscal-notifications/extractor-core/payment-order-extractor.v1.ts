...
export type PaymentOrderDocumentKindV1 = "CARTA_DE_PAGO" | "DOCUMENTO_DE_INGRESO";
...
const LABELS = Object.freeze({
  paymentReference: ["numero de justificante", "justificante del ingreso", "referencia de pago"],
  ...
  taxId: ["n.i.f.", "nif"],
  recipient: ["nombre y apellidos / razon social", "nombre o razon social", "obligado al pago"],
  ...
  paymentChannel: ["medio de pago", "forma de pago", "modalidad de pago", "lugar de pago"],
  collaboratingEntity: ["entidad colaboradora", "entidad financiera"],
  bankAccount: ["cuenta de cargo", "iban"],
  barcodeReference: ["referencia del codigo de barras", "codigo de barras", "referencia bancaria"],
...
const MONEY_LABELS = Object.freeze({
  ...
  TOTAL_DUE: ["importe total", "total a ingresar", "importe a ingresar", "importe del ingreso", "importe pendiente", "total"],
} as const satisfies Readonly<Record<PaymentOrderMoneyRoleV1, readonly string[]>>);
...
export function extractPaymentOrderV1(input: ExtractPaymentOrderInputV1): PaymentOrderExtractorOutputV1 {
  ...
  const parsed = parsePaymentOrder(input.document.documentId, recognition.documentKind, lines);
  const evidencePages = uniqueNumbers([...paymentPages]);
  const evidence = createEvidence(input.document, validated.pageToFactSegment, evidencePages);
  ...
  const positiveTotal = parsed.facts.moneyFacts.find((fact) =>
    fact.role === "TOTAL_DUE" && fact.sign === "POSITIVE" && fact.amountCents > 0
  );
  if (positiveTotal) {
    const positiveDebtMoney = Object.freeze(parsed.money.filter((component) => component.sign === "POSITIVE"));
...
function recognizePaymentOrder(
...
): { readonly status: "RECOGNIZED"; readonly documentKind: PaymentOrderDocumentKindV1 } |
  ...
  const headerLines = lines.filter((line) =>
    line.pageNumber === firstPage && line.lineIndex < PAYMENT_ORDER_EXTRACTOR_LIMITS_V1.maxHeaderLines
  );
  ...
  const trustedSegmentAuthority = paymentSegments.every((segment) =>
  ...
  paymentSegments.forEach((segment) => {
    ...
    if (title === "carta de pago" || title.startsWith("carta de pago ")) titles.add("CARTA_DE_PAGO");
    if (title === "documento de ingreso" || title.startsWith("documento de ingreso ")) titles.add("DOCUMENTO_DE_INGRESO");
  });
  lines.forEach((line) => {
    if (line.folded === "carta de pago") titles.add("CARTA_DE_PAGO");
    if (line.folded === "documento de ingreso") titles.add("DOCUMENTO_DE_INGRESO");
  });
  if (titles.size > 1) return { status: "BLOCKED", warning: "CONFLICTING_PAYMENT_ORDER_KIND" };
...
function parsePaymentOrder(
  ...
  const paymentReference = uniqueLabelFact(lines, LABELS.paymentReference, "Número de justificante", warnings, "CONFLICTING_PAYMENT_REFERENCE");
  ...
  const recipient = uniqueLabelFact(lines, LABELS.recipient, "Obligado al pago", warnings, "CONFLICTING_RECIPIENT");
  const taxConcept = uniqueLabelFact(lines, LABELS.taxConcept, "Concepto tributario", warnings, "CONFLICTING_TAX_CONCEPT");
  const rawPaymentDeadline = uniqueLabelFact(lines, LABELS.paymentDeadline, "Vencimiento de pago", warnings, "CONFLICTING_PAYMENT_DEADLINE");
  const paymentChannel = uniqueLabelFact(lines, LABELS.paymentChannel, "Medio de pago", warnings, "CONFLICTING_PAYMENT_CHANNEL");
  ...
  const barcodeReference = uniqueLabelFact(lines, LABELS.barcodeReference, "Referencia bancaria", warnings, "CONFLICTING_BARCODE_REFERENCE");
  ...
  const explicitNrc = uniqueLabelFact(lines, LABELS.nrc, "NRC", warnings, "CONFLICTING_NRC");
  if (explicitNrc && /^[A-Za-z0-9]{22}$/u.test(explicitNrc.printedValue.replaceAll(" ", ""))) {
    warnings.push("PAYMENT_EVIDENCE_PRESENT_REQUIRES_SEPARATE_EXTRACTOR");
  }
  if (!paymentReference && !liquidationKey) warnings.push("MISSING_EXPLICIT_PAYMENT_REFERENCE");
  if (!moneyFacts.some((fact) => fact.role === "TOTAL_DUE")) warnings.push("MISSING_EXPLICIT_TOTAL_DUE");
  if (!rawPaymentDeadline) warnings.push("MISSING_EXPLICIT_PAYMENT_DEADLINE");

  const references: ReferenceV1[] = [];
  addReference(references, documentId, "OTHER_OFFICIAL_REFERENCE", paymentReference);
...
function extractMoneyFacts(
  ...
  (Object.entries(MONEY_LABELS) as readonly [PaymentOrderMoneyRoleV1, readonly string[]][]).forEach(([role, labels]) => {
    ...
    if (!only) return;
    if (role === "TOTAL_DUE" && (only.sign !== "POSITIVE" || only.cents === 0)) {
...
function extractLabelValue(
  ...
  if (sameLine.length > 0 && sameLine.length <= PAYMENT_ORDER_EXTRACTOR_LIMITS_V1.maxTextFactChars) return sameLine;
...
function textFact(
  ...
  if (printedValue.length === 0 || printedValue.length > PAYMENT_ORDER_EXTRACTOR_LIMITS_V1.maxTextFactChars) {
    throw new FiscalNotificationInputError("INVALID_INPUT", "paymentOrder.textFact");
  }
  return Object.freeze({
    printedValue,
...
function validateSegments(
  ...
  if (!Array.isArray(rawSegments) || rawSegments.length === 0 || rawSegments.length > document.pages.length) {
    throw new FiscalNotificationInputError("COLLECTION_LIMIT_EXCEEDED", "paymentOrder.segments");
  }
  ...
  for (const segment of segments) {
    if (segment.documentId !== document.documentId || ids.has(segment.segmentId)) {
      throw new FiscalNotificationInputError("INVALID_INPUT", "paymentOrder.segments.identity");
    }
    ids.add(segment.segmentId);
    for (let page = segment.pageFrom; page <= segment.pageTo; page += 1) {
      if (covered.has(page) || page > document.pages.length) {
        throw new FiscalNotificationInputError("INVALID_INPUT", "paymentOrder.segments.coverage");
      }
      covered.add(page);
  ...
  for (const page of document.pages) {
    ...
    for (let index = 0; index < pageLines.length; index += 1) {
      ...
      if (raw.length > PAYMENT_ORDER_EXTRACTOR_LIMITS_V1.maxLineChars) {
        throw new FiscalNotificationInputError("INVALID_INPUT", `paymentOrder.pages[${page.pageNumber}].line`);
      }
      ...
      if (lines.length > PAYMENT_ORDER_EXTRACTOR_LIMITS_V1.maxLines) {
        throw new FiscalNotificationInputError("COLLECTION_LIMIT_EXCEEDED", "paymentOrder.lines");
      }
    }
  }
...
function createEvidence(
  ...
  pages.forEach((page) => {
    ...
    if (!segment) throw new FiscalNotificationInputError("INVALID_INPUT", "paymentOrder.evidence.segment");
...
function moneyComponentType(role: PaymentOrderMoneyRoleV1): MonetaryComponentTypeV1 {
  switch (role) {
    case "PRINCIPAL": return "PRINCIPAL";
    case "SURCHARGE": return "SURCHARGE";
    case "EXECUTIVE_SURCHARGE": return "EXECUTIVE_SURCHARGE";
    case "LATE_INTEREST": return "LATE_INTEREST";
    case "COSTS": return "COSTS";
    case "TOTAL_DUE": return "TOTAL_CLAIMED";
  }
}
...
export const PAYMENT_ORDER_EXTRACTOR_RELEASE_V1 = Object.freeze({
  ...
  officialInterpretationSources: Object.freeze([
    ...
    Object.freeze({ sourceId: "aeat.payment.liquidations.card", url: "https://sede.agenciatributaria.gob.es/Sede/ayuda/consultas-informaticas/pago-impuestos-deudas-tasas-ayuda-tecnica/pago-liquidaciones-deudas-mediante-tarjeta.html" }),
