...
function snapshot(
  ...
  return buildListVisualCacheSnapshot(createDemoWorkspaceData(date), kind, date);
}
...
describe("list visual cache", () => {
  ...
  it("lee por scope, descarta cache caducada o malformada y no lanza", () => {
    ...
    expect(readListVisualCacheSnapshot("facturas", "user-2", storage)).toBeNull();
    expect(
      readListVisualCacheSnapshot(
        "facturas",
        scope,
  ...
  it("detecta cambios visuales sin depender de la fecha de guardado", () => {
    ...
    const sameDataLater = snapshot("gastos", new Date("2026-07-25T11:00:00.000Z"));
    const changed = {
      ...sameDataLater,
