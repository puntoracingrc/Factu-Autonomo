import type { TaxModelNumber, TaxRule } from "./contracts";
...
const BLUEPRINTS: readonly RuleBlueprint[] = [
  { modelNumber: "035", conditions: ["Alta, modificación o baja en un régimen OSS/IOSS"], exclusions: ["No hay ventas B2C europeas ni uso del régimen"], result: "Revisar comunicación censal OSS/IOSS", officialSourceIds: ["aeat.model-369.procedure"] },
  { modelNumber: "036", conditions: ["Alta, baja o modificación censal", "Inicio de operaciones intracomunitarias o cambio de obligación"], exclusions: ["No se ha producido ningún cambio censal"], result: "Declaración censal por evento y fecha de efecto", officialSourceIds: ["aeat.roi-intraeu", "aeat.models.catalog"] },
  ...
  { modelNumber: "130", conditions: ["Persona física en estimación directa"], exclusions: ["Estimación objetiva", "Excepción del 70 % aplicable a la actividad"], result: "Pago fraccionado de IRPF en estimación directa", officialSourceIds: ["aeat.model-130.instructions"] },
  { modelNumber: "131", conditions: ["Persona física en estimación objetiva"], exclusions: ["Estimación directa", "Excepción del 70 % agraria, ganadera o forestal cuando proceda"], result: "Pago fraccionado de IRPF en estimación objetiva", officialSourceIds: ["aeat.model-131.instructions"] },
  ...
  { modelNumber: "200", conditions: ["El sujeto que factura es contribuyente del Impuesto sobre Sociedades"], exclusions: ["La actividad factura exclusivamente como persona física o entidad en atribución"], result: "Declaración anual de la sociedad", officialSourceIds: ["aeat.models.catalog"] },
  ...
  { modelNumber: "308", conditions: ["Supuesto específico de devolución previsto por la normativa"], exclusions: ["Mera existencia de cuotas de IVA o resultado a devolver del 303"], result: "Solicitud especial de devolución a revisar", officialSourceIds: ["aeat.models.catalog"] },
  { modelNumber: "309", conditions: ["Sujeto sin autoliquidación periódica que realiza una operación no periódica declarable"], exclusions: ["La operación ya se liquida en 303"], result: "Autoliquidación no periódica de IVA", officialSourceIds: ["aeat.model-303.instructions-2026", "aeat.roi-intraeu"] },
  { modelNumber: "341", conditions: ["Reintegro de compensaciones en el régimen agrario"], exclusions: ["No existe el supuesto especial"], result: "Declaración especial a revisar", officialSourceIds: ["aeat.models.catalog"] },
  ...
  { modelNumber: "349", conditions: ["Operaciones intracomunitarias comprendidas realizadas en el período"], exclusions: ["Alta en ROI sin operaciones", "Operaciones exentas excluidas del concepto informable"], result: "Declaración recapitulativa de operaciones intracomunitarias", officialSourceIds: ["aeat.roi-intraeu"] },
  { modelNumber: "369", conditions: ["Operaciones declaradas mediante un régimen OSS/IOSS"], exclusions: ["No existe adhesión al régimen ni operación comprendida"], result: "Declaración periódica del régimen de ventanilla única", officialSourceIds: ["aeat.model-369.procedure"] },
  { modelNumber: "390", conditions: ["Autoliquidaciones periódicas de IVA y ausencia de exoneración"], exclusions: ["Exoneración del resumen anual aplicable al ejercicio"], result: "Resumen anual de IVA", officialSourceIds: ["aeat.model-390.procedure"] },
  { modelNumber: "714", conditions: ["Circunstancias patrimoniales que superan las reglas autonómicas y estatales"], exclusions: ["No se alcanza obligación según residencia y ejercicio"], result: "Revisión personal del Impuesto sobre el Patrimonio", officialSourceIds: ["aeat.models.catalog"] },
  { modelNumber: "720", conditions: ["Bienes o derechos en el extranjero dentro de categorías y límites informativos"], exclusions: ["No se alcanzan umbrales o concurre una exclusión"], result: "Revisión personal de información de bienes en el extranjero", officialSourceIds: ["aeat.models.catalog"] },
  { modelNumber: "721", conditions: ["Monedas virtuales custodiadas en el extranjero dentro de la obligación"], exclusions: ["No existe custodia extranjera o no se alcanza la obligación"], result: "Revisión personal de monedas virtuales en el extranjero", officialSourceIds: ["aeat.models.catalog"] },
] as const;
...
export const TAX_RULES: readonly TaxRule[] = YEARS.flatMap((fiscalYear) =>
  BLUEPRINTS.map((blueprint) => ({
    ...
    version: `es-common.${fiscalYear}.2026-07-14.v1`,
    fiscalYear,
...
export function taxRuleSetVersion(fiscalYear: 2025 | 2026): string {
  return `es-common.${fiscalYear}.2026-07-14.v1`;
}
