...
import {
  ...
  type CentralInvoiceAuthorityHistoricalImportRpcArgs,
  type CentralInvoiceAuthorityHistoricalImportRpcClient,
} from "@/lib/central-invoice-authority/historical-import-route-handler";
...
const routeHandler = createCentralInvoiceAuthorityHistoricalImportRouteHandler({
  ...
  async rateLimit(request, userId) {
    const result = await checkRateLimit(
      ...
      {
        namespace: "central_invoice_authority_historical_import",
        limit: 5,
        windowMs: 10 * 60_000,
      },
      userId,
    ...
    return {
      rpc(
        ...
        return admin.rpc(name, args) as unknown as ReturnType<
          CentralInvoiceAuthorityHistoricalImportRpcClient["rpc"]
        >;
      },
    };
  },
    ...
    await routeHandler.handle({
      ...
      readBody: async () => {
        const body = await readTextBody(request, {
          maxBytes: MAX_HISTORICAL_IMPORT_BODY_BYTES,
          invalidMessage: "JSON invalido",
          tooLargeMessage: "Importacion historica central demasiado grande",
        });
        if (!body.ok) {
          throw new Error(
