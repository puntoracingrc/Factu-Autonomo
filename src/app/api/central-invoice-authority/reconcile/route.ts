...
} from "@/lib/central-invoice-authority/account-series-reconciliation-route-handler";
import type {
  CentralInvoiceAuthorityAccountSeriesReconciliationRpcArgs,
  CentralInvoiceAuthorityAccountSeriesReconciliationRpcClient,
} from "@/lib/central-invoice-authority/account-series-reconciliation-rpc";
...
const MAX_RECONCILIATION_BODY_BYTES = 64 * 1024;
  ...
  createCentralInvoiceAuthorityAccountSeriesReconciliationRouteHandler({
    ...
    async rateLimit(request, userId) {
      const result = await checkRateLimit(
        ...
        {
          namespace: "central_invoice_authority_reconcile",
          limit: 10,
          windowMs: 10 * 60_000,
        },
        userId,
    ...
    async verifyDevice({ userId, sessionId, token, userAgent }) {
      ...
      return {
        allowed: true,
        deviceId: hashCloudDeviceToken(normalizedToken),
      };
    },
    getRpcClient() {
      ...
      return {
        rpc(
          ...
          return admin.rpc(name, args) as unknown as ReturnType<
            CentralInvoiceAuthorityAccountSeriesReconciliationRpcClient["rpc"]
          >;
        },
      };
    },
    ...
    await routeHandler.handle({
      ...
      readBody: async () => {
        const body = await readTextBody(request, {
          maxBytes: MAX_RECONCILIATION_BODY_BYTES,
          invalidMessage: "JSON invalido",
          tooLargeMessage: "Conciliacion de series demasiado grande",
        });
        if (!body.ok) {
          throw new Error(
