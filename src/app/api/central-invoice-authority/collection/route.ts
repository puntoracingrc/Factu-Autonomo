...
} from "@/lib/central-invoice-authority/collection-rpc-adapter";
...
const routeHandler = createCentralInvoiceAuthorityCollectionRouteHandler({
  ...
  async rateLimit(request, userId) {
    const result = await checkRateLimit(
      ...
      {
        namespace: "central_invoice_authority_collection",
        limit: 60,
        windowMs: 10 * 60_000,
      },
      userId,
    ...
    return {
      rpc(
        ...
        return admin.rpc(name, args) as unknown as ReturnType<
          CentralInvoiceAuthorityCollectionRpcClient["rpc"]
        >;
      },
    };
  },
    ...
    await routeHandler.handle({
      ...
      readBody: async () => {
        const body = await readTextBody(request, {
          maxBytes: MAX_COLLECTION_BODY_BYTES,
          invalidMessage: "JSON invalido",
          tooLargeMessage: "Solicitud de cobro central demasiado grande",
        });
        if (!body.ok) {
          throw new Error(
