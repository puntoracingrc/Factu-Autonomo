...
import type {
  ...
  CentralBusinessNumberedDocumentRpcClient,
} from "@/lib/central-business-authority/numbered-document-rpc-adapter";
...
const handler = createCentralBusinessNumberedDocumentRouteHandler({
  ...
  async rateLimit(request, userId) {
    const result = await checkRateLimit(
      ...
      {
        namespace: "central_business_numbered_document",
        limit: 60,
        windowMs: 10 * 60_000,
      },
      userId,
    ...
    return {
      rpc(
        ...
        return admin.rpc(name, args) as unknown as ReturnType<
          CentralBusinessNumberedDocumentRpcClient["rpc"]
        >;
      },
    };
  },
    ...
    await handler.handle({
      ...
      readBody: async () => {
        const body = await readTextBody(request, {
          ...
          tooLargeMessage: "Documento central demasiado grande",
        });
        if (!body.ok) {
          throw new Error(
