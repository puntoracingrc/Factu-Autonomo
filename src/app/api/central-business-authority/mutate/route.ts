...
} from "@/lib/central-business-authority/mutation-rpc-adapter";
...
const handler = createCentralBusinessMutationRouteHandler({
  ...
  async rateLimit(request, userId) {
    const result = await checkRateLimit(
      ...
      {
        namespace: "central_business_authority_mutate",
        limit: 120,
        windowMs: 10 * 60_000,
      },
      userId,
    ...
    return {
      rpc(
        ...
        return admin.rpc(name, args) as unknown as ReturnType<
          CentralBusinessMutationRpcClient["rpc"]
        >;
      },
    };
  },
  reserveQuota: reserveCentralBusinessMutationQuota,
  commitQuota: commitCentralQuotaReservations,
  releaseQuota: releaseCentralQuotaReservations,
  releaseDeletedQuota: releaseDeletedCentralBusinessQuota,
});
    ...
    await handler.handle({
      ...
      readBody: async () => {
        const body = await readTextBody(request, {
          ...
          tooLargeMessage: "Mutacion central demasiado grande",
        });
        if (!body.ok) {
          throw new Error(
