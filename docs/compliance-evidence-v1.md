...
| Gate release autoridad central | Bloquear releases que pierdan la clasificación viva entre Git y Supabase producción antes de autoridad central. | `npm run check:authority-central-release-gate` | Encadena `check:migrations`, `validate:central-invoice-authority-migration-gap-classification`, `validate:central-invoice-authority-migration-gap-operational-classification` y `validate:central-invoice-authority-phase2`; no introduce SQL productivo ni cambia runtime. | CI `Quality`; evidencias `docs/architecture/supabase-production-migration-gap-classification-2026-07-24.json` y `docs/architecture/central-invoice-authority-migration-gap-operational-classification-2026-07-25.json`; readiness gates de autoridad central. | Requisito operativo previo a cualquier SQL productivo o activación de autoridad central; Supabase Acceptance sigue como check obligatorio separado. |
