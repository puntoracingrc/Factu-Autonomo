...
4. permite observar cuentas concretas con
   `CENTRAL_INVOICE_AUTHORITY_SHADOW_USER_EMAILS=<email>[,<email>]`: el endpoint
   puede comprobar servidor y series, pero devuelve `effectiveMode: "shadow"` y
   nunca habilita escrituras fiscales; si una cuenta aparece tambien en el
   canario privado, prevalece el canario;
5. anade `NEXT_PUBLIC_CENTRAL_INVOICE_AUTHORITY_FORM_REQUIRED=true` como guardia
   publica de fail-closed si se decide activar required en una build;
6. consulta `/api/central-invoice-authority/status` para los canarios publicos y
   cuando no hay bandera publica pero el documento es candidato a autoridad
   central;
7. entra al camino central si el servidor declara `requestedMode: "required"`;
8. entra al camino central con razon `public_form_canary` solo si el canario
   publico esta pedido y el servidor declara `summary.fiscalWritesPossible`;
9. entra al camino central con razon `server_fiscal_writes_possible` si no hay
   flag publico y el servidor declara `summary.fiscalWritesPossible`;
10. mantiene el flujo local si no hay sesion/dispositivo, el modo esta `off`,
...
11. sigue dejando que `/issue` haga el preflight final antes de escribir.
12. muestra un aviso no bloqueante en formularios candidatos para que el usuario
   vea si el canario central esta activo, en espera o sin comprobacion antes de
   pulsar emitir;
13. documenta las variables de activacion en `.env.example` siempre apagadas por
    defecto.
...
No cambia variables de Vercel por si solo, no aplica migraciones remotas y no
activa `CENTRAL_INVOICE_AUTHORITY_MODE`. Solo prepara la politica runtime
compartida y su aviso visible para que una activacion posterior no dependa de
recordar el flag de canary ni de emitir a ciegas desde el formulario.
