...
-- Extend the audited, owner-locked bootstrap to user reminders without
-- changing its transaction, idempotency, command, entity or outbox contracts.
...
comment on function public.bootstrap_central_business_entities_v1(
  ...
  'Atomically imports an exact local customer, supplier, product and user reminder snapshot after fail-closed conflict checks.';
