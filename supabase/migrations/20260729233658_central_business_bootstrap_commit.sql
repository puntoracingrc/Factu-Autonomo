...
-- Audited, all-or-nothing bootstrap for legacy local customers, suppliers and
-- products. Normal mutations and bootstrap commits share one owner lock.
...
create table if not exists public.central_business_bootstraps (
  ...
  central_state_digest text not null,
  ...
  constraint central_business_bootstraps_hashes_v1 check (
    idempotency_key_hash ~ '^[0-9a-f]{64}$'
    and request_hash ~ '^[0-9a-f]{64}$'
    and snapshot_digest ~ '^[0-9a-f]{64}$'
    and central_state_digest ~ '^[0-9a-f]{64}$'
    and preview_digest ~ '^[0-9a-f]{64}$'
  ),
  constraint central_business_bootstraps_entity_count_v1 check (
    entity_count between 0 and 5000
  ),
  constraint central_business_bootstraps_actor_v1 check (
    actor_device_id <> '' and actor_session_hash <> ''
  ),
  constraint central_business_bootstraps_status_v1 check (
    status in ('pending', 'committed')
  ),
  constraint central_business_bootstraps_completion_v1 check (
    (
      status = 'pending'
      and created_count is null
      and identical_count is null
      and first_event_sequence is null
    ...
    or (
      ...
      and created_count >= 0
      and identical_count is not null
      and identical_count >= 0
      and created_count + identical_count = entity_count
      and (
        (
          created_count = 0
          and first_event_sequence is null
        ...
        or (
          ...
          and first_event_sequence <= last_event_sequence
        )
      )
...
create unique index if not exists central_business_bootstraps_idempotency_uidx
  on public.central_business_bootstraps (user_id, idempotency_key_hash);

create index if not exists central_business_bootstraps_owner_requested_idx
  on public.central_business_bootstraps (user_id, requested_at desc);

alter table public.central_business_bootstraps enable row level security;

revoke all on table public.central_business_bootstraps from public, anon, authenticated;
grant all on table public.central_business_bootstraps to service_role;

drop policy if exists central_business_bootstraps_deny_clients_v1
  on public.central_business_bootstraps;
create policy central_business_bootstraps_deny_clients_v1
  on public.central_business_bootstraps
  as restrictive
  for all
...
create or replace function public.central_business_lock_owner_before_command_v1()
returns trigger
language plpgsql
set search_path = ''
as $$
...
revoke all on function public.central_business_lock_owner_before_command_v1()
  from public, anon, authenticated;
grant execute on function public.central_business_lock_owner_before_command_v1()
  to service_role;

drop trigger if exists central_business_commands_owner_lock_bi_v1
  on public.central_business_commands;
create trigger central_business_commands_owner_lock_bi_v1
  before insert on public.central_business_commands
  for each row
  execute function public.central_business_lock_owner_before_command_v1();
...
begin
  ...
  if exists (
    ...
    where pg_catalog.jsonb_typeof(item.value) <> 'object'
      ...
      or coalesce(item.value -> 'payload' ->> 'id', '')
  ...
  if exists (
    ...
    where central.user_id = p_user_id
      and central.entity_type in ('customer', 'supplier', 'product')
...
comment on function public.bootstrap_central_business_entities_v1(
  ...
  'Atomically imports an exact local customer, supplier and product snapshot after fail-closed conflict checks.';
