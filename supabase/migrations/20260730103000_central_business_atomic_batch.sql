-- CENTRAL_BUSINESS_ATOMIC_BATCH_V1
-- Executes distinct entity mutations inside one PostgreSQL transaction.
...
begin
  ...
  -- Stable ordering prevents two overlapping batches from taking locks in
  -- opposite order. Any exception below rolls the whole function call back.
  for v_item in
    select
...
comment on function public.mutate_central_business_batch_v1(
  ...
  'Applies 1-20 distinct central business mutations atomically with stable lock ordering and per-operation idempotency.';
