...
-- Keeps each bundle atomic while allowing one supplier plus up to 99 expenses.
...
begin
  ...
  -- Stable ordering prevents overlapping batches from taking locks in opposite
  -- order. Any exception below rolls the whole function call back.
  for v_item in
    select
...
comment on function public.mutate_central_business_batch_v1(
  ...
  'Applies 1-100 distinct central business mutations atomically with stable lock ordering and per-operation idempotency.';
