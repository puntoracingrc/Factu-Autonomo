...
-- Scope: preserve V2 collection semantics while qualifying every relation
-- reference that can collide with RETURNS TABLE output variables in PL/pgSQL.
...
comment on function public.update_central_invoice_collection_v1(
  ...
  'CENTRAL_INVOICE_AUTHORITY_COLLECTION_STATUS_EVENTS_V3 server-only operational collection update with relation-qualified PL/pgSQL references.';
