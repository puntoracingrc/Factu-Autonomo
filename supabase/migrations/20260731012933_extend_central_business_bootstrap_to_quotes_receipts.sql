...
-- Extend the audited, owner-locked bootstrap to quotes and receipts without
-- opening the fiscal invoice lane or weakening transaction, idempotency,
-- command, entity or outbox contracts.
...
begin
  ...
  if exists (
    ...
    where pg_catalog.jsonb_typeof(item.value) <> 'object'
      ...
      or (
        item.value ->> 'entityType' = 'quote'
        and (
          coalesce(item.value -> 'payload' ->> 'type', '') <> 'presupuesto'
          or item.value -> 'payload' ? 'centralInvoiceAuthority'
          or item.value -> 'payload' ? 'rectification'
          or item.value -> 'payload' ? 'verifactu'
        )
      )
      or (
        item.value ->> 'entityType' = 'receipt'
        and (
          coalesce(item.value -> 'payload' ->> 'type', '') <> 'recibo'
          or item.value -> 'payload' ? 'centralInvoiceAuthority'
          or item.value -> 'payload' ? 'rectification'
          or item.value -> 'payload' ? 'verifactu'
        )
...
comment on function public.bootstrap_central_business_entities_v1(
  ...
  'Atomically imports an exact local customer, supplier, product, reminder, expense, recurring expense, quote, receipt and singleton profile snapshot after fail-closed conflict checks; fiscal invoices remain excluded.';
