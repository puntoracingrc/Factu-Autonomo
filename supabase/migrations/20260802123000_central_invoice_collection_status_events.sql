...
-- Scope: synchronize operational collection status for centrally issued
-- invoices without changing fiscal identity, emitted snapshots or hashes.
...
begin
  ...
  if jsonb_typeof(v_payload_document) <> 'object'
    or v_payload_document->>'id' is distinct from v_document.local_document_id
...
comment on function public.update_central_invoice_collection_v1(
  ...
  'CENTRAL_INVOICE_AUTHORITY_COLLECTION_STATUS_EVENTS_V1 server-only operational collection status update for centrally issued invoices.';
