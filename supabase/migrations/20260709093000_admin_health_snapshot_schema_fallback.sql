...
declare
  ...
  v_has_error_events boolean := to_regclass('public.app_error_events') is not null;
  v_has_customer_ai_usage boolean := exists (
    select 1
    from information_schema.columns c
    where c.table_schema = 'public'
      and c.table_name = 'user_usage'
      and c.column_name = 'customer_ai_autofills_created'
  );
  v_usage jsonb;
  v_errors jsonb;
  v_hourly jsonb;
begin
  ...
  if v_has_customer_ai_usage then
