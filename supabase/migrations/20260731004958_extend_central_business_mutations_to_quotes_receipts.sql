...
-- Quotes and receipts use the same versioned, idempotent transaction as the
-- other business entities. Fiscal invoices remain outside this authority.

begin;

alter table public.central_business_entities
  drop constraint central_business_entities_type_v1,
  add constraint central_business_entities_type_v1 check (
    entity_type in (
      'customer',
      'supplier',
      'product',
...
alter table public.central_business_commands
  ...
  add constraint central_business_commands_type_v1 check (
    entity_type in (
      'customer',
      'supplier',
      'product',
...
alter table public.central_business_outbox
  drop constraint central_business_outbox_type_v1,
  add constraint central_business_outbox_type_v1 check (
    entity_type in (
      'customer',
      'supplier',
      'product',
