...
begin
  if exists (
    ...
    where entity_type = 'expense'
      and deleted = false
      and nullif(
        pg_catalog.btrim(current_payload ->> 'recurringOccurrenceKey'),
        ''
      ) is not null
    group by
      user_id,
    ...
    raise exception using
      errcode = 'P4105',
      message = 'duplicate central recurring occurrence requires repair';
  end if;
end;
$$;
...
comment on function public.mutate_central_business_entity_v1(
  ...
  'Versioned central mutation with P4102 idempotency, P4103 version, P4104 not-found and P4105 recurring occurrence conflict codes.';
